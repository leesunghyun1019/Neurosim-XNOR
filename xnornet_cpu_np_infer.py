...
class XConv2d(nn.Conv2d):
    ...
    def forward(self, input):  
        if self.inference == 1:
            ...
            binary_input = binactive(input)
            
            #scaling factor K
            A = BinActiv.Mean(input)  
            ...
            k=torch.ones(1,1,self.kernel_size,self.kernel_size).mul(1/(self.kernel_size**2)).to(input.device)
            K = F.conv2d(A, k, stride=self.stride, padding=self.padding)#K의 size은 (64,1,28,28)
            
            #scaling factor alpha(Per-Filter Scaling Factor)
            n=self.weight[0].nelement()
            alpha = (
                    self.weight.abs()  # Weight의 절댓값 계산
                    .sum(dim=(1, 2, 3), keepdim=True)  # 각 필터에 대한 절댓값의 합
                    .div(n)  # n으로 나누어 평균 계산
                    .expand_as(self.weight)  # Weight 텐서 크기에 맞게 확장
                    )
            ...
            output = F.conv2d(binary_input, binary_weights, self.bias, self.stride, self.padding, self.dilation, self.groups) #x의 크기는 (64,128,28,28)
            output = output.mul(K)
            output = wage_quantizer.LinearQuantizeOut(output, self.ADCprecision) #추가
